...
0. **Enrollment** (`playbooks/enroll_vm.yml`, triggered per new instance -- e.g. by an EventBridge rule on `ec2:RunInstances`): confirms the SSM Agent that shipped with the instance's AMI is actually registered and `Online` (a control-plane check, not a remote install -- see the role's README for why installing it via Ansible is structurally impossible here), then tags the instance `Monitoring: disk`. This is what makes step 2 find it at all.
...
2. **Inventory**: `ansible-playbook -i inventory/accounts/ ...` loads every file in that directory. Each file's `amazon.aws.aws_ec2` plugin uses that account's profile, which makes boto3 transparently do `AnsibleAutomationRole` → `AnsibleSpokeRole` double-hop assumption, and lists every EC2 instance tagged `Monitoring: disk` (step 0's output).
